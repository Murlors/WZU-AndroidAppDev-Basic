...
public class PinyinUtils {
    ...
    private static ArrayList<String> convertSplit2String(
        ...
        Collections.addAll(out, array0);
        for (int i = 1; i < splits.size(); i++) {
            int pre_out_size = out.size();
            String[] arrays = splits.get(i);
            if (arrays.length > 1) {
                ArrayList<String> temp = new ArrayList<>(out);//将out的值复制给temp
            ...
            if (pending > 128) {
                ...
                try {
                    ...
                    if (pys != null) {
                        ...
                        Collections.addAll(pySet, pys);
                        list.add(pySet.toArray(new String[0]));
                    }
